...
public class DividerItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private void setOrientation(@OrientationType int orientation) {
        if (orientation != LinearLayoutManager.VERTICAL && orientation != LinearLayoutManager.HORIZONTAL) {
            throw new IllegalArgumentException("Incoming layout type is illegal");
        }
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        // use this to draw the divider line
    ...
    private void drawHorizontal(Canvas c, RecyclerView parent) {
        ...
        int chiddCount = parent.getChildCount();
        for (int i = 0; i < chiddCount; i++) {
            View child = parent.getChildAt(i);
